...
export default function StaffRegisterPage() {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 to-teal-50 px-4">
        <div className="w-full max-w-md rounded-2xl bg-white p-8 text-center shadow-lg ring-1 ring-slate-100">
          ...
          <h1 className="mt-4 text-xl font-bold text-slate-900">Request received</h1>
          <p className="mt-2 text-sm text-slate-600">
            An administrator will review and approve your account. Once approved, you'll get an email with a temporary password.
          </p>
          <Link to="/login" className="mt-6 inline-block rounded-lg bg-teal-600 px-5 py-2.5 font-semibold text-white hover:bg-teal-700">
            Back to sign in
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-slate-50 to-teal-50 px-4 py-10">
      <form onSubmit={handleSubmit} className="w-full max-w-md space-y-4 rounded-2xl bg-white p-8 shadow-lg ring-1 ring-slate-100">
        <div className="flex flex-col items-center">
          ...
          <h1 className="mt-3 text-xl font-bold text-slate-900">Staff account request</h1>
          <p className="text-sm text-slate-500">Approved by an administrator before access</p>
        </div>
        {error && <div className="rounded-lg bg-red-50 px-4 py-3 text-sm text-red-700 ring-1 ring-red-100">{error}</div>}
        <div>
        ...
        <p className="text-center text-sm text-slate-500">
          <Link to="/login" className="text-teal-600 hover:underline">Back to sign in</Link>
        </p>
      </form>
    </div>
  );
